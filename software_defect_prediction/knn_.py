...
combined_model.fit([X_train_lstm, X_train_scaled], y_train, epochs=10, batch_size=64, validation_split=0.2)

# Extract features using the trained LSTM and DNN models
X_train_features = np.concatenate((lstm_model.predict(X_train_lstm), dnn_model.predict(X_train_scaled)), axis=1)
X_test_features = np.concatenate((lstm_model.predict(X_test_lstm), dnn_model.predict(X_test_scaled)), axis=1)
